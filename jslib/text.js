...
function parseMarkdown(buildLink, md) {
    ...
    const tokens = [
        new Token('link', /\[(?<linkName>.+?)\]\((?<linkHref>.+?)\)/, groups => buildLink(groups.linkName, groups.linkHref)),
        new Token('paragraph', /(?<paragraph>\n\n)/, groups => document.createElement('br')),
        new Token('boldItalic', /\*\*\*(?<boldItalictext>[^*]+?)\*\*\*/, groups => {
        ...
        new Token('bold', /\*\*(?<boldText>[^*]+?)\*\*/, groups => {
            const b = document.createElement('b');
            b.innerText = groups.boldText;
            return b;
        }),
        new Token('italic', /\*(?<italicText>[^*]+?)\*/, groups => {
            const i = document.createElement('i');
            i.innerText = groups.italicText;
            return i;
        }),
        new Token('heading', /(?<h>#{1,6}) (?<headingText>[^\n]*)\n/, groups => {
